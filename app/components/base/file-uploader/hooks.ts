...
import { SupportUploadFileTypes, TransferMethod } from '@/types/app'
import type { FileUpload } from './types'
import { formatFileSize } from './utils'
import type { FileUploadConfigResponse } from './types'
import { uploadRemoteFile, type RemoteFileResponseType } from '@/service'
...
export const useFile = (fileConfig: FileUpload) => {
  ...
  const handleLoadFileFromLink = useCallback((url: string) => {
    ...
    uploadRemoteFile(url).then((res) => {
      const mimeType = res.mime_type ? res.mime_type : SupportUploadFileTypes.custom
      const newFile: FileEntity = {
        ...uploadingFile,
        type: mimeType,
        size: res.size,
        progress: 100,
        supportFileType: getSupportFileType(res.name, mimeType, allowedFileTypes?.includes(SupportUploadFileTypes.custom)),
        uploadedId: res.id,
        url: res.url,
      }
      if (!isAllowedFileExtension(res.name, mimeType, fileConfig.allowed_file_types || [], fileConfig.allowed_file_extensions || [])) {
